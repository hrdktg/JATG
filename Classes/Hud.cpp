...
void Hud::reduceHp(bool obj) { //Player ->0
    hp[obj]--;
    if(hp[obj] < 0) {
        CCLOG("Kill obj");
    }
...
void Hud::initFullHpBar() {
    ...
    for(int x=0;x<hp[0];x++) {
        hp_bars[0].push_back(getFromSheet("heart_player.png"));
        hp_bars[0][x]->setPosition(cocos2d::Vec2(90+x*60,507));
        sc->addChild(hp_bars[0][x]);
    }

    for(int x=0;x<hp[1];x++) {
        hp_bars[1].push_back(cocos2d::Sprite::createWithSpriteFrameName("heart_enemy.png"));
        hp_bars[1][x]->setPosition(cocos2d::Vec2(1610+x*60,760));
